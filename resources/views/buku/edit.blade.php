...
<form action="{{route('buku.update', $buku->id)}}" method="POST" enctype="multipart/form-data">
    ...
    <div class="container-data">
        ...
        <div class="form-row">
            ...
            <input type="text" class="form-control" name="genre" placeholder="Genre" value="{{ $buku->genre}}">
            </div>
            <div class="form-group col-md-6">
            <label for="id_kategori">Kategori</label>
            <select name="id_kategori" class="form-control">
                <option  value="{{ $buku->kategoris->id }}" selected>{{ $buku->kategoris->nama_kategori }}</option>
        ...
        <div class="form-group">
            ...
            <textarea name="sinopsis" id="summernote" rows="10" class="form-control">{{ $buku->sinopsis}}</textarea>
        </div>
       
        <div class="form-group">
            <label for="foto">Upload Sampul</label>
            <input type="file" name="foto" class="form-control" placeholder="image">
            <img src="/thumb/{{ $buku->foto }}" width="200px"> 
        </div> 
